...
//     try {
//       const extracted = extractPolygonsFromDXF(
//         dataMap[uiStore.activeKey || "data"]
//       );
...
//       setSecondSelectedPolygon(temp.map((p) => [p.x, p.y]));
...
//       setDebugRays(rays);
//       setIntersectingPolygons(allIntersectingPolygons);
//       baseplateStore.polygons = allIntersectingPolygons;
...
//       intersectingPolys.forEach((intersectingPoly) => {
...
//         if (Math.abs(closestInner.distance) < THRESHOLD) {
...
//           const color = index === 0 ? "lightgreen" : "lightgreen";
//           drawPolygon(ctx, polygon, color, 3);
//         }
//       });
...
const CanvasTest = observer(() => {
  ...
  // Function to filter polygons by dimension
        ...
        for (let j = 0; j < allIntersectingPolygons.length; j++) {
          ...
          if (isContained) {
            ...
            if (polyArea < otherPolyArea) {
              return false; // Skip smaller polygon
            }
          }
        }
        return true; // Keep larger polygon or non-contained polygons
      });
  ...
  const traceAllPolygonsWithRays = (startPoly, otherPolys) => {
    ...
    // Add the starting polygon to the set of intersecting polygons
    ...
    while (queue.length > 0) {
      ...
      // Filter intersecting polygons by dimension, using startPoly as reference
  ...
  const draw = () => {
    ...
    if (baseplateStore.polygons.length > 0) {
      ...
      newRectangles.forEach((rect) => {
        drawPolygon(ctx, rect, "blue", 2); // Unified color to blue
      });
