...
// Constants for dimension balloons and offset (adjust based on drawing units)
const TEXT_HEIGHT_WORLD = 0.0025; // 2.5 mm if units are meters
const BALLOON_RADIUS_WORLD = 0.01; // 1 mm if units are meters
const OFFSET_WORLD = 0.03; // 3 mm for balloon offset if units are meters
const DIMENSION_OFFSET_WORLD = 0.05; // 5 mm offset for dimension lines if units are meters
const MIN_FONT_SIZE = 8; // Minimum font size in pixels for readability
...
const CanvasTest: React.FC = observer(() => {
  ...
  const drawDimensionLine = (
    ...
    // Calculate midpoint and direction in world space
    const midWorld = {
    ...
    // Calculate perpendicular direction for balloon offset
    const perpX = -dy / len;
    const perpY = dx / len;

    // Balloon center position
    ...
    // Draw leader line from midpoint to balloon center
  ...
  const draw = () => {
    ...
    if (baseplateStore.polygons.length > 0) {
      ...
      // Draw dimensions for visible elements with offset
            ...
            (wall) => {
              if (wall && wall.length > 0) {
                for (let i = 0; i < wall.length; i++) {
                  ...
                  const perpX = -dy / len;
                  const perpY = dx / len;
                  // Offset outward (assuming clockwise polygon, adjust if needed)
                  drawDimensionLine(
                    ctx,
                    p1,
                    p2,
