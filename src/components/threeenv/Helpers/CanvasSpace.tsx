...
//     const PURPLE_OFFSET = 0.75;
//     const MAGENTA_WIDTH = 18.50;
//     const MAGENTA_HEIGHT = 21.00;

//     const blueRectangles = baseplateStore.polygons.map((baseplate) => {
...
//       const edges = [
//         { start: { x: minX, y: minY }, end: { x: maxX, y: minY }, dir: { x: 0, y: -1 } }, // Bottom
//         { start: { x: maxX, y: minY }, end: { x: maxX, y: maxY }, dir: { x: 1, y: 0 } }, // Right
//         { start: { x: maxX, y: maxY }, end: { x: minX, y: maxY }, dir: { x: 0, y: 1 } }, // Top
//         { start: { x: minX, y: maxY }, end: { x: minX, y: minY }, dir: { x: -1, y: 0 } }, // Left
//       ];
...
//           const rayEnd = {
//             x: rayStart.x + edge.dir.x * CONFIG.RAY_LENGTH,
//             y: rayStart.y + edge.dir.y * CONFIG.RAY_LENGTH,
//           };
...
//             const hit = doLinesIntersect(rayStart, rayEnd, v, w, CONFIG.EPSILON);
//             if (hit) {
...
//     const purpleRectangles = blueRectangles.map((blueRect) => {
//       const bounds = getBounds(blueRect);
//       return [
//         { x: bounds.minX - PURPLE_OFFSET, y: bounds.minY - PURPLE_OFFSET },
//         { x: bounds.maxX + PURPLE_OFFSET, y: bounds.minY - PURPLE_OFFSET },
//         { x: bounds.maxX + PURPLE_OFFSET, y: bounds.maxY + PURPLE_OFFSET },
//         { x: bounds.minX - PURPLE_OFFSET, y: bounds.maxY + PURPLE_OFFSET },
//       ];
//     });

//     const magentaRectangles = purpleRectangles.map((purpleRect) => {
//       const bounds = getBounds(purpleRect);
//       const centerX = (bounds.minX + bounds.maxX) / 2;
//       const centerY = (bounds.minY + bounds.maxY) / 2;
//       const halfWidth = MAGENTA_WIDTH / 2;
//       const halfHeight = MAGENTA_HEIGHT / 2;
//       return [
...
//     columnStore.setPolygons(blueRectangles);
//     foundationStore.setInnerPolygons(purpleRectangles);
//     foundationStore.setOuterPolygons(magentaRectangles);

//     return { blue: blueRectangles, purple: purpleRectangles, magenta: magentaRectangles };
//   }, [
//     wallStore.externalWallPoints,
//     wallStore.internalWallPoints,
//     baseplateStore.polygons,
...
//       columnStore.polygons.forEach((rect) => {
//         drawPolygon(ctx, rect, "blue", 2);
//       });

//       foundationStore.innerPolygons.forEach((rect) => {
//         drawPolygon(ctx, rect, "magenta", 2);
//       });

//       foundationStore.outerPolygons.forEach((rect) => {
//         drawPolygon(ctx, rect, "magenta", 2);
//       });
...
//       rectangles.blue.forEach((rect) => {
//         drawPolygon(ctx, rect, "blue", 2);
//       });

//       rectangles.purple.forEach((rect) => {
//         drawPolygon(ctx, rect, "magenta", 2);
//       });

//       rectangles.magenta.forEach((rect) => {
//         drawPolygon(ctx, rect, "magenta", 2);
//       });
...
const CanvasTest = observer(() => {
  ...
  const structures = useMemo(() => {
    ...
    const OUTER_FOUNDATION_WIDTH = 18.5;
    const OUTER_FOUNDATION_HEIGHT = 21.0;
    ...
    const columns = baseplateStore.polygons.map((baseplate) => {
      ...
      const originalBounds = { minX: bounds.minX, maxX: bounds.maxX, minY: bounds.minY, maxY: bounds.maxY };
      const newBounds = { minX, maxX, minY, maxY };
    ...
    const mullionColumns = columns
      .map((column) => {
        ...
        const extendedEdge = verticalEdges.find((edge) =>
          edge.start.y !== edge.end.y &&
          (edge.start.y < bounds.minY || edge.end.y > bounds.maxY)
        ) || verticalEdges[0];

        const wallMidpoint = getWallMidpoint(outerWall, innerWall, extendedEdge);
        const mullionCenter = {
        ...
        return [
          { x: mullionCenter.x - halfThickness, y: mullionCenter.y - halfThickness },
          { x: mullionCenter.x + halfThickness, y: mullionCenter.y - halfThickness },
          { x: mullionCenter.x + halfThickness, y: mullionCenter.y + halfThickness },
          { x: mullionCenter.x - halfThickness, y: mullionCenter.y + halfThickness },
        ];
  ...
  const draw = () => {
    ...
    if (baseplateStore.polygons.length > 0) {
      ...
      structures.innerFoundation.forEach((inner) => {
        drawPolygon(ctx, inner, "purple", 2);
      });
