...
if (!("standards" in uiStore)) {
  ...
  store.editStandard = function (
    id: number,
    updatedStandard: Partial<Standard>
  ) {
    const index = this.standards.findIndex((t: Standard) => t.id === id);
    if (index != -1) {
      this.standards[index] = { ...this.standards[index], ...updatedStandard };
    }
  };
}
