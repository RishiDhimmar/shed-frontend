...
function TextDrawer({ texts, scale = 1 }) {
      ...
      {texts.map((text) => (
        <>
          <Text
            ...
            x={
              text.height ? text.position.x - text.height / 2 : text.position.x
            } // Scale x position
            y={text.width ? text.position.y - text.height / 2 : text.position.y} // Scale y position
