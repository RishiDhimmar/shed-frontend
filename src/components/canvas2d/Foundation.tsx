...
//      return foundationStore.addFoundationToGroup(newGroupName, foundationLabel);
//   };
...
//   const handleChangeOfType = (value, groupName, foundationLabel) => {
//     console.log(groupName)
//     foundationStore.changeFoundationType(groupName, foundationLabel, value);
//   };
...
//                   name={foundationKey}
//                   onDblClick={() => {
//                     const grpName = handleMoveToNewGroup(group.name, foundation.label);
//                     handleChangeOfType(
//                       "foundation",
//                       grpName,
//                       foundation.label
...
import uiStore from "../../stores/UIStore";
import foundationStore from "../../stores/FoundationStore";
import { Line, Text, Group, Circle, Rect } from "react-konva";
import { observer } from "mobx-react-lite";
...
const Foundation = observer(() => {
  ...
  const handleMoveToNewGroup = (groupName, foundationLabel) => {
    const newGroupName = `Group ${foundationStore.groups.length + 1}`;
    const newGroup = {
      name: newGroupName,
      foundations: [],
    };
  ...
  const openModal = (groupName, foundationLabel, outerBbox) => {
    setModalState({
      ...
      foundationLabel,
      position: {
        x: outerBbox.minX + 2000, // Apply 5000 offset to x
        y: outerBbox.minY,
      },
    });
  };
  ...
  return (
    <>
            ...
            group.foundations.map((foundation, j) => {
              ...
              return (
                <Group
                  ...
                  name={foundationKey}
                  onDblClick={() =>
                    openModal(group.name, foundation.label, outerBbox)
                  }
                >
                  <Line
                    points={foundation.innerFoundationPoints.flatMap((p) => [
      ...
      <FoundationTypeModal
        ...
        onClose={closeModal}
        onSelect={handleChangeOfType}
        groupName={modalState.groupName}
        foundationLabel={modalState.foundationLabel}
